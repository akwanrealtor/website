const express = require('express');
const fs = require('fs');
const path = require('path');

const ensureLogos = () => {
  const assets = [
    {
      filePath: path.join(__dirname, 'public', 'images', 'LOGO-green.png'),
      base64:
        '',
    },
    {
      filePath: path.join(__dirname, 'public', 'images', 'logo-favicon.png'),
      base64:
        '',
    },
  ];

  for (const { filePath, base64 } of assets) {
    try {
      if (!fs.existsSync(filePath)) {
        fs.mkdirSync(path.dirname(filePath), { recursive: true });
        fs.writeFileSync(filePath, Buffer.from(base64, 'base64'));
      }
    } catch (error) {
      console.error(`Failed to create ${path.basename(filePath)}:`, error);
    }
  }
};

const app = express();
const PORT = process.env.PORT || 3000;

app.use(express.static(path.join(__dirname, 'public')));

app.use((req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'index.html'));
});

app.listen(PORT, () => {
  console.log(`Server listening on port ${PORT}`);
});

module.exports = app;
